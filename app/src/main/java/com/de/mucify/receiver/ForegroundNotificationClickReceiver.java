...
public class ForegroundNotificationClickReceiver extends BroadcastReceiver {
    ...
    public void onReceive(Context context, Intent intent) {
        if(intent.hasExtra("PlayPause") && intent.getAction().equals(SongPlayForegroundService.ACTION_PLAY_PAUSE)) {
        ...
        else if(intent.getAction().equals(SongPlayForegroundService.ACTION_PREVIOUS))
            AudioController.get().previous(context);
        else if(intent.getAction().equals(SongPlayForegroundService.ACTION_NEXT))
